...
Source/sample/environment/site standardization expansion:

- A genus-level unique-value inventory was generated for standardization planning, scanning 5,066 genus metadata files and 2,596,460 rows across 637 selected metadata columns.
- Column curation scope was formalized in `fetchm_webapp/data/standardization_unique_values/genus_metadata_column_classification.csv`: 70 columns were classified as primary standardized fields, 190 as secondary recovery fields, 37 as optional later curation fields, 285 as raw-only, and 55 as excluded from curation.
- Standardized output columns were expanded beyond the previous `Isolation_Source_SD`, `Sample_Type_SD`, and `Environment_Medium_SD` fields. FetchM now also writes `Isolation_Site_SD`, `Environment_Broad_Scale_SD`, `Environment_Local_Scale_SD`, `Host_Disease_SD`, and `Host_Health_State_SD`, each with broad/detail/method/ontology companion fields.
- Controlled-category loading now supports the new destinations directly instead of forcing all source-like rules through the sample-type dictionary. `Isolation_Source_SD` rules are stored separately, reducing leakage of food/source labels into `Sample_Type_SD`.
- Conservative source/sample/environment/site rules were generated from the genus unique-value inventory in `source_sample_environment_site_conservative_rules_20260430.csv`. The rule set added 5,880 non-duplicate mappings to `controlled_categories.csv`: 2,287 `Environment_Medium_SD`, 1,897 `Sample_Type_SD`, 1,771 `Isolation_Source_SD`, 434 `Isolation_Site_SD`, 277 `Environment_Broad_Scale_SD`, 98 `Environment_Local_Scale_SD`, 95 `Host_Disease_SD`, and 11 `Host_Health_State_SD` rules.
- High-count correction rules were added for values such as `sterile site`, `sputum`, `raw intact chicken`, `pathogen.cl`, `whole organism`, `hospital`, `clinical`, `wound`, `gut`, `environmental`, `lung`, `respiratory`, `skin`, `CSF`, `nasopharyngeal`, `Digestive tract enviroment`, `urban`, and animal-source context phrases.
- Remaining non-missing, high-count values requiring review are stored in `source_sample_environment_site_review_queue_20260430.csv`. These are intentionally not auto-approved when they are ontology IDs, numeric/provenance values, host-only labels in source fields, or ambiguous long-tail free text.
- A genus-only standardization refresh was queued for 5,066 taxa, representing an estimated 2,595,579 rows, so existing managed metadata can be rewritten with the expanded standardized fields.
